...
from utils.dataset import load_data_sc
...
if __name__=='__main__':
    ...
    # prepare the output
    args.out_path = os.path.join(args.out_path, 'hcformer_sc')
    ...
    if args.use_wandb:
        if args.use_sweep:
            sweep_name = 'hcformer_sc'+str(args.num)
            sweep_configuration = {
                'project': 'hcformer_sc',
                'method': 'random',
                'name': sweep_name,
            ...
            sweep_id = wandb.sweep(sweep=sweep_configuration, project='hcformer_sc')
            wandb.agent(sweep_id, function=train)
        elif args.parallelize:
            args.model_save_path = os.path.join(args.model_save_path, 'hcformer_sc'+str(args.num))
